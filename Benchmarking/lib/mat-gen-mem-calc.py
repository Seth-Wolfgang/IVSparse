...
def compute_mmr(spmat, bytes_per_index=4, bytes_per_val=8, debug=False):
    ...
    for i in range(len(spmat.indptr) - 1):
        colvals = spmat.data[spmat.indptr[i]:spmat.indptr[i + 1]]
        # print('colvals:', colvals, len(colvals))
        nnz_col = len(colvals)
    ...
    avg_difflog /= (len(spmat.indptr) - 1)  # new metric
    mmr_avg /= (len(spmat.indptr) - 1)
    return mmr_avg, vcsc, ivcsc_size, avg_difflog, (bytes_needed.min(), bytes_needed.max(), bytes_needed.mean())
